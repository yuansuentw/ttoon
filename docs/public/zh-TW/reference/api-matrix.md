...
所有三個 TTOON SDK 都提供完全一致的 API 表面：在 Rust、JavaScript 和 Python 之間達到 **18/18** 的完全一致 (parity)。

## 分類後的 API 參考頁

詳細 API 參考改為依工作型態分類，而不是依語言拆分：

- **[T-TOON 批次 API](./ttoon-batch-api.md)** — 以 T-TOON 文字為中心的批次讀寫與轉碼 API
- **[T-JSON 批次 API](./tjson-batch-api.md)** — 以 T-JSON 文字為中心的批次讀寫與轉碼 API
- **[Stream API](./stream-api.md)** — 兩種格式共用的串流讀寫 API
...
轉碼只會經過 Rust 的內部表示 (IR) — 它不會具現化為特定語言的原生物件。所有的 typed 語意 (decimal, uuid 等等) 皆會完整保留。
...
| 註冊 codec | — (使用原生型別) | `use(codecs)` | `use(codecs)` |
...
codec 註冊被排除在 18/18 計數外，因為它是 JS/Python 專屬的；此矩陣只計算在三種 SDK 中都存在的 API。
