...
JS 對 `decimal`、`date`、`time`、`datetime` 和 `uuid` 預設回傳 `string`。這是為了避免將使用者綁定到特定的第三方依賴。可透過 `use()` 註冊 codec 來改變此行為。
...
- Decimal `m` 後綴是否存在
- T-TOON 字串 escape 合規性（僅允許 5 種）
- T-JSON 物件鍵必須為字串
- Arrow 路徑的欄位型別一致性

## T-TOON 與 T-JSON 的 Escape 差異

T-TOON 僅支援 5 種 escape 序列：`\\`、`\"`、`\n`、`\r`、`\t`。T-JSON 支援完整的 JSON escape 集，包含 `\uXXXX`、`\b`、`\f`。在 T-TOON 中使用 T-JSON 的 escape 序列會導致解析錯誤。
