...
| `invalid escape sequence in T-TOON string` | 使用了不支援的跳脫序列（如 `\uXXXX`） | T-TOON 僅允許 `\\` `\"` `\n` `\r` `\t`；完整跳脫支援請使用 T-JSON |
| `unknown delimiter` / `unknown binary_format` | 傳入了不支援的選項值 | delimiter 僅接受 `","`, `"\t"`, `"|"`；binary_format 僅接受 `"hex"`, `"b64"` |

## 解析模式相關問題 (Parse Mode Issues)
...
`mode` 參數對 T-JSON 解析沒有影響 — T-JSON 一律為嚴格模式。這是刻意設計，因為 T-JSON 遵循 JSON 的結構規則。

## Arrow 相關問題 (Arrow Issues)
...
- 透過 `use()` 註冊的編解碼器是全域的，請確保 `await use(...)` 完成後再進行解析
- 單次呼叫時可在 `ParseOptions` 的 `codecs` 中覆寫全域設定，僅對該次呼叫生效
- 編解碼器不影響 Arrow 路徑 — 僅作用於物件路徑（`parse()` / `stringify()`）
...
## Python 特定問題 (Python-Specific Issues)
...
- 或直接將 Arrow 表格傳給 `read_arrow()` 或 `stringify_arrow_tjson()`
